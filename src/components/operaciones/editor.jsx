...
const Edit = () => {
        ...
        for (let i = 1; i < arr.length - 1; i++) {
          ...
          for (let j = 0; j < arr[0].length; j++) {
            ...
            if (arr[i][j] === undefined) {
              ...
              if (j === 3) {
                periodo =
                  moment(arr[i][j], "DD/MM/YYYY").format("MMMM") +
                  " " +
                  moment(arr[i][j], "DD/MM/YYYY").year();
              }
  ...
  const mesAtras = () => {
    let miNewPeriodo = moment(miPeriodo, "MMMM/YYYY")
      .subtract(1, "months")
      .format("MMMM YYYY");
  ...
  return (
    <div>
      <div className="editorHeader">
        ...
        <div className="actions">
          <Dropdown
            ...
            placeholder="Filtrar Checkpoint"
            clearable
            multiple
            selection
            value={filtro}
            options={optionsOnSelect}
            onChange={(e, { value }) => setFiltro(value)}
          />
          <Button
            className="buttonCSV"
            inverted
      ...
      <Tabla
        ...
        filtro={filtro}
        reset={() => setFiltro([])}
        datosfiltro={(data) => setOptions(data)}
      />
    </div>
  );
};
