...
const Tabla = (props) => {
  ...
  useEffect(() => {
    db.collection("refas2")
      ...
      .onSnapshot((data) => {
        ...
        filterByProperty([...misRefacciones], "periodo", props.periodo).then(
          (data) => {
            setData(data);
            setDelMes(data);
            readOptions(data);
  ...
  // Filtrar por periodo
  useEffect(() => {
    filterByProperty([...refacciones], "periodo", props.periodo).then(
      (data) => {
        setData(data);
        setDelMes(data);
        readOptions(data);
  ...
  const abrirModalEditarRef = (refa) => {
    setOpenModal(true);
    setPropsModal(refa);
  };
              ...
              data.map((line, i) => (
                ...
                >
                  ...
                  <li>
                    {line["Creation_date"] === ""
                      ? ""
                      : moment(line["Creation_date"]).format("DD MMM YY HH:mm")}
                  </li>
                  <li>{line["Carrier"]}</li>
                  <li>{line["Waybill"]}</li>
                  ...
                  <li>
                    {line["Fecha Arribo"] === ""
                      ? ""
                      : moment(line["Fecha Arribo"]).format("DD MMM")}
                  </li>
                  <li>{line["Guia local"]}</li>
                  <li>
                    {line["Ship Date"] === ""
                      ? ""
                      : moment(line["Ship Date"]).format("DD MMM")}
                  </li>
                  <li>
                    {line["Delivery date"] === ""
                      ? ""
                      : moment(line["Delivery date"]).format("DD MMM")}
                  </li>
